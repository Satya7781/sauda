...
print("[MAIN] Importing database module...", file=sys.stderr)
try:
    import database as db_mod
    print("[MAIN] Database module imported successfully", file=sys.stderr)
except Exception as e:
    print(f"[MAIN] ERROR: Failed to import database: {e}", file=sys.stderr)
    raise

print("[MAIN] Creating FastAPI app instance...", file=sys.stderr)
app = FastAPI(title="Sauda API")
print("[MAIN] FastAPI app created successfully", file=sys.stderr)
...
def startup_event():
    try:
        ...
        try:
            ...
            print(f"[STARTUP] Warning: Seeding skipped ({str(e)})")
    except Exception as e:
        print(f"[STARTUP] Warning: Database init skipped ({str(e)})")
...
def create_product(product_data: dict, db: Session = Depends(get_db)):
    seller_id = product_data.get("seller") or product_data.get("sellerId")
    ...
    seller = db.query(db_mod.Seller).filter(db_mod.Seller.id == seller_id).first()
    if not seller:
        raise HTTPException(status_code=404, detail="Seller not found")

    category = db.query(db_mod.Category).filter(db_mod.Category.id == category_id).first()
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")

    new_product = db_mod.Product(
        title=title,
        title_hi=(product_data.get("titleHi") or title).strip(),
        title_en=(product_data.get("titleEn") or title).strip(),
        title_mr=(product_data.get("titleMr") or title).strip(),
        price=float(product_data.get("price") or 0),
        unit=(product_data.get("unit") or "pcs").strip(),
        seller_id=seller_id,
    ...
    return {
        ...
        "titleHi": new_product.title_hi,
        "titleEn": new_product.title_en,
        "titleMr": new_product.title_mr,
        "price": new_product.price,
        "unit": new_product.unit,
        "seller": new_product.seller_id,
