...
function showVoiceSearchResults(products, query) {
  ...
  if (products.length === 0) {
    container.innerHTML = '<div class="p-6 text-center"><p class="text-sm font-bold" style="color:var(--text2)">"\'' + query + '\'" ke liye koi item nahi mila</p><p class="text-xs mt-2" style="color:var(--text3)">Kripya doosre shabd se dhundhiye</p></div>';
  } else {
    container.innerHTML = '<div class="p-2 text-xs font-semibold" style="color:var(--text3)">Aapke liye ' + products.length + ' item(s) available hain:</div>' + 
      products.map(function (p) {
        var sellerRec = getSellerRecord(p.seller);
        var sellerName = sellerRec ? (sellerRec.name || p.seller) : (p.seller || 'Seller');
        var sellerLocality = sellerRec ? (sellerRec.locality || '') : '';
        var catObj = getCategoryRecord(p.category) || null;
        var icon = catObj ? catObj.icon : 'fa-solid fa-box';
        var availStatus = (p.stock && p.stock > 0) ? '✓ Available' : 'Out of Stock';
        var statusColor = (p.stock && p.stock > 0) ? '#10b981' : '#ef4444';
        return '<div class="s-card p-4 mb-3 flex items-start gap-3 cursor-pointer hover:shadow-lg transition-all" onclick="closeVoiceOverlay();openProductDetail(\'' + p.id + '\')">' +
          '<div class="w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 text-lg" style="background:var(--accent-light);color:var(--accent)"><i class="fa-solid ' + icon + '"></i></div>' +
          '<div class="flex-1 min-w-0">' +
            '<p class="text-sm font-bold truncate">' + getProductTitle(p) + '</p>' +
            '<p class="text-xs mt-1" style="color:var(--text3)">' + sellerName + (sellerLocality ? ', ' + sellerLocality : '') + '</p>' +
            '<div class="flex items-center gap-2 mt-2"><p class="text-base font-extrabold" style="color:var(--accent);font-family:\'Space Grotesk\',sans-serif">\u20B9' + p.price + '</p>' +
            '<span class="text-[10px] px-2 py-1 rounded" style="background:' + statusColor + ';color:white">' + availStatus + '</span></div>' +
          '</div>' +
        '</div>';
      }).join('');
  }
...
async function publishProductItem(itemData) {
...
function setupVoiceOverlayEvents() {
  ...
  if (publishBtn) publishBtn.addEventListener('click', function () {
    if (!generatedItemData) return;
    showToast('Publishing...');
    publishProductItem(generatedItemData);
  });

  var editBtn = document.getElementById('voice-ol-edit-btn');
  if (editBtn) editBtn.addEventListener('click', function () {
    if (!generatedItemData) return;
    switchMode('manual');
    document.getElementById('voice-ol-manual-title').value = generatedItemData.title;
    document.getElementById('voice-ol-manual-title-hi').value = generatedItemData.titleHi || '';
    document.getElementById('voice-ol-manual-cat').value = generatedItemData.category;
    document.getElementById('voice-ol-manual-price').value = generatedItemData.price;
    document.getElementById('voice-ol-manual-unit').value = generatedItemData.unit;
    document.getElementById('voice-ol-manual-stock').value = generatedItemData.stock;
  });
